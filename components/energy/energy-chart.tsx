...
export function EnergyChart({ data }: { data: EnergyPoint[] }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      {/* left: 0, not negative -- a negative margin here pushes the Y-axis tick labels to a
          negative x coordinate inside the chart's own SVG, which gets silently clipped by the
          SVG's default overflow:hidden viewport. Barely visible on a wide desktop card; on a
          narrow mobile one it cropped the leading digit off every tick ("100" rendered as "00"). */}
