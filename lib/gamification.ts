...
async function hasEventToday(userId: string, type: XpEventType): Promise<boolean> {
 ...
 *  prize level, and runs the badge-criteria check. Safe to call liberally — every rule here is
 *  idempotent (daily cap, skipDuplicates on badge/prize creation). */
export async function awardXp(userId: string, type: XpEventType): Promise<AwardResult> {
  if (type === "daily_login" && (await hasEventToday(userId, type))) {
...
/** Evaluates every badge's unlock criteria against current data and awards any newly-earned
 *  ones. "Consistent N months" counts distinct logged periods rather than verifying they're
 *  consecutive calendar months — a deliberate simplification, not a correctness-critical rule. */
export async function checkAndAwardBadges(userId: string): Promise<BadgeDef[]> {
  const existing = await prisma.badge.findMany({ where: { userId }, select: { badgeType: true } });
  ...
  if (!has("hemat_energi")) {
    const lastTwo = await prisma.energyLog.findMany({
      where: { userId },
      orderBy: { period: "desc" },
      take: 2,
    });
  ...
  if (!has("warga_setia")) {
    const logins = await prisma.xpEvent.findMany({
